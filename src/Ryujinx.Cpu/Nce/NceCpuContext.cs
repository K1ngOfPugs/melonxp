    ...
    class NceCpuContext : ICpuContext
    {
        ...
        public void Execute(IExecutionContext context, ulong address)
        {
            ...
            try
            {
                ...
                ulong execAddress = address;
                
                if (_memoryManager is MemoryManagerNative nativeMemoryManager)
                {
                    ulong physicalAddress = nativeMemoryManager.GetPhysicalAddress(address);
                    
                    Logger.Info?.Print(LogClass.Cpu, $"MemoryManagerNative detected:");
                    Logger.Info?.Print(LogClass.Cpu, $"  Virtual address:  0x{address:X16}");
                    Logger.Info?.Print(LogClass.Cpu, $"  Physical address: 0x{physicalAddress:X16}");
                    Logger.Info?.Print(LogClass.Cpu, $"  Page table ptr:   0x{nativeMemoryManager.PageTablePointer:X}");
                    execAddress = physicalAddress;
                }
                else
                {
                    Logger.Debug?.Print(LogClass.Cpu, $"Using virtual address directly: 0x{address:X16}");
                }
                ...
                nec.SetStartAddress(execAddress);
