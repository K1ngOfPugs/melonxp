...
namespace Ryujinx.Cpu.Nce
{
    ...
    public class NceCpuCodePatch
    {
        ...
        {
            if (memoryManager is MemoryManagerNative nativeMM)
            {
                // patchAddress = nativeMM.GetPhysicalAddress(patchAddress);
                // textAddress = nativeMM.GetPhysicalAddress(textAddress);
            }
            ...
            foreach (var patchTarget in _patchTargets)
            {
                ...
                memoryManager.Reprotect(instTextAddress & ~0xFFFUL, 0x1000, MemoryPermission.ReadAndWrite);
            ...
            Console.WriteLine($"Cool :3 {Environment.StackTrace}");
