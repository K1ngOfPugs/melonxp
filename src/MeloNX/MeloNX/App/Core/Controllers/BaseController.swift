...
class BaseController: Equatable, Identifiable {
    ...
    func tryRegisterMotion(slot: UInt8) {
        if playerSlot == slot && tryGetMotionProvider() != nil {
            return
        }
        
        playerSlot = slot
        
        // Clean up existing providers if slot changed
        if let _ = controllerMotionProvider {
            controllerMotionProvider = nil
        }
        if let _ = deviceMotionProvider {
            deviceMotionProvider = nil
        }
